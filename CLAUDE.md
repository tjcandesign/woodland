...
7. **Confirm the `woodlandteam@woodlandtitle.com` mailbox exists** (footer/contact email was switched from the `-dc` domain; mail will bounce if the box isn't provisioned). Note: `Send Earnest Money` still points at `woodlandtitledc.paymints.io` — left as-is intentionally; confirm that's still correct.
8. Brand-tint the **Qualia widget** in the Qualia dashboard to match the palette (token already embedded site-wide via `app/(site)/layout.tsx`).
9. Decide final fate of internal pages (keep `noindex`, password-gate, or remove).
10. (Optional) Model the bespoke home/security prose in Sanity; consolidate the two brand pages into one.

**Sanity is fully wired + seeded (90 docs) and content edits flow to the site on rebuild. Site is LIVE.**
